...
exports.cart_list = async function(req, res) {
  ...
  res.locals.links.push({ name: 'Giỏ hàng', route: '/shoppingcart' });
  var total = 0;
  var products = [];
  if (!req.user) {
    if (!req.session.cart) {
      return res.render('cart/shoppingcart', {
        ...
        products: products,
        user: req.user,
        mess: 'Chưa có sản phẩm nào trong giỏ hàng.'
      });
    }

    await Promise.all(
      req.session.cart.map(async element => {
        var result = await Product.findOne(
          { _id: element.product },
          { _id: 0, name: 1, price: 1, img: 1 }
        );
    ...
    await Promise.all(
      req.user.cart.map(async element => {
        var result = await Product.findOne(
          { _id: element.product },
          { _id: 0, name: 1, price: 1, img: 1 }
        );
...
exports.cart_update_product = async function(req, res) {
  ...
  if (!req.user) {
    ...
    await Promise.all(
      req.session.cart.map(async element => {
        ...
        if (element.amount !== 0) {
          ...
          req.session.cart = req.session.cart.filter(pro => {
            return pro.amount !== 0;
          });
        }
      })
    ...
    user.cart = req.user.cart.filter(pro => {
      return pro.amount !== 0;
    });
    await user.save();
  }
